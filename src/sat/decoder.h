...
public:
    ...
    Plan extractPlan() {
        ...
        recursiveCreateHierarchy(classicalPlan, plan, 0, 0);
    ...
    const USignature& getOpTrue(int layer, int pos) {
        for (const auto& [s, v] : _layers.at(layer)->at(pos).getVariableTable(VarType::OP)) {
    ...
    void recursiveCreateHierarchy(std::vector<PlanItem>& classicalPlan, std::vector<PlanItem>& hierarchy, int layer, int pos) {

        int true_layer;
        int true_pos;

        // Get the true layer and position true (since we reuse the same position in multiple layers)
        true_layer = _layers.at(layer)->at(pos).getLayerIndex();
        true_pos = _layers.at(layer)->at(pos).getPositionIndex();

        layer = true_layer;
        pos = true_pos;

        // Get the action/reduction true for this layer position
        const USignature& actionOrReductionTrue = getOpTrue(layer, pos);
        ...
        if (_htn.isAction(actionOrReductionTrue)) {
            ...
            int v = _vars.getVariable(VarType::OP, layer, pos, actionOrReductionTrue);

            // Log::i("%s:%s @ (%i,%i)\n", TOSTR(r.getTaskSignature()), TOSTR(actionOrReductionTrue), layer, pos);
            USignature decRSig = getDecodedQOp(layer, pos, actionOrReductionTrue);
            ...
            // Get the decomposition size of this reduction
            int numDecompositions = r.getSubtasks().size();

            if (numDecompositions == 0) return;

            // Save a set of layer/position which will be called recursively
            std::set<std::pair<int, int>> layerPosToCall;

            int successorPos = _layers.at(layer)->getSuccessorPos(pos);
            // Get all the successors position of this layer/position (there will be )
            for (int i = 0; i < numDecompositions; i++) {
                int nextPos = successorPos + i;
                int nextLayer = layer + 1;
                // Get the id of the action/reduction true for this layer position

                const USignature& nextActionOrReductionTrue = getOpTrue(nextLayer, nextPos);
            ...
            for (auto& [l, p] : layerPosToCall) {
                recursiveCreateHierarchy(classicalPlan, hierarchy, l, p);
            }

        } else {
            Log::e("Plan error: Invalid action/reduction id=%i at %i,%i!\n", actionOrReductionTrue._name_id, layer, pos);
